<template>
  <div class="w-full flex flex-wrap justify-around text-left" v-if="shouldShow">
    ...
    <div class="px-3 column">
      <input
        class="w-full form-control form-input form-input-bordered py-3 px-4"
        :class="{'border-danger': v.label.$error}"
        type="text"
        :id="id+'_label'"
    ...
    <div class="px-3 column">
      <select
        ...
        @change="checkField"
      >
        <option v-for="(field, key) in fields" :value="field.name" :key="key">{{ field.name }}</option>
      </select>
      ...
      <modal ref="modalRelations" v-if="modalRelations" :width="800" :align="'flex justify-end'">
        <div slot="container">
          <div class="w-full flex flex-wrap">
            <div class="w-1/3 pr-3">
              <label
                ...
                :for="id+'_relation'"
              >{{ __('Relation type') }}</label>
              
              <select
                class="w-full form-control form-input form-input-bordered px-4"
                :class="{'border-danger': v.relation.$error}"
                :id="id+'_relation'"
              ...
              >
                ...
                <option
                  v-for="(relation, key) in columnRelations"
                  :value="key"
                  :key="key"
                >{{ relation }}</option>
              </select>
            ...
            <div class="w-1/3 pr-3">
              <label
                class="block uppercase tracking-wide text-grey-darker text-xs font-bold mb-2"
                :for="id+'_relation_table'"
              >{{ __('Related table') }}</label>
              <select
                class="w-full form-control form-select"
                :id="id+'_relation_table'"
                :class="{'border-danger': v.related_table.$error}"
                v-model="column.related_table"
                @change="changeTableRelated"
              >
                <option
                  disabled="disabled"
                  selected="selected"
                  :value="null"
                >{{ __('Choose a table') }}</option>
                <option
                  v-for="info in tables"
                  :value="info.table"
                  :key="info.table"
                >{{ info.table }}</option>
              </select>
            ...
            <div class="w-1/3 pr-3">
              <label
                class="block uppercase tracking-wide text-grey-darker text-xs font-bold mb-2"
                :for="id+'_relation_column'"
              >{{ __('Related column') }}</label>
              <select
                class="w-full form-control form-select"
                :id="id+'_relation_column'"
                :class="{'border-danger': v.related_column.$error}"
                v-model="column.related_column"
              >
                <option
                  disabled="disabled"
                  selected="selected"
                  :value="null"
                >{{ __('Choose a column') }}</option>
                <option
                  v-for="info in related_columns"
                  :value="info.name"
                  :key="info.name"
                >{{ info.name }}</option>
              </select>
          ...
          <div class="w-full flex flex-wrap mt-6" v-if="column.relation == 'belongsTo'">
            <label
              class="block uppercase tracking-wide text-grey-darker text-xs font-bold mr-2"
              :for="id+'_relation_column_searchable'"
            >{{ __('Searchable') }}</label>
            <input
              type="checkbox"
              :id="id+'_relation_column_searchable'"
        ...
        <div slot="buttons">
          ...
          <button
            ...
            v-on:click="modalRelations = false"
          >{{ __('Set') }}</button>
        </div>
      </modal>
    </div>
    ...
    <div class="px-3 column">
      <select
        ...
        v-model="column.show"
      >
        <option v-for="(showText, key) in selectShow" :value="key" :key="key">{{ showText }}</option>
      </select>
    </div>
    ...
    <div class="px-3 column" v-if="hasExtraColumn">
      ...
      <modal ref="modalExtras" v-if="modalExtras" :width="extraPopupWidth">
        <div slot="container">
            ...
            <div class="flex flex-wrap">
              <div class="w-1/2 flex flex-wrap content-start">
                ...
                <div class="w-full pt-3">
                  <button
                    class="mx-auto btn btn-default btn-primary mr-3"
                    @click="addOption"
            ...
            <div class="flex flex-wrap">
              <div class="w-full flex flex-wrap content-start">
                <div class="w-full pr-3">
                  <label
                    class="block uppercase tracking-wide text-grey-darker text-xs font-bold mb-2"
                    :for="id+'_code_language'"
                  >{{ __('Language or Json') }}</label>
                  <select
                    class="w-full form-control form-select"
                    :id="id+'_code_language'"
                    v-model="column.language"
                  >
                    <option
                      disabled="disabled"
                      selected="selected"
                    ...
                    <option
                      v-for="(language, key) in codeSelect"
                      :value="key"
                      :key="key"
                    >{{ language }}</option>
                  </select>
            ...
            <div class="flex flex-wrap">
              <div class="w-full flex flex-wrap content-start">
                <div class="w-1/2 pr-3">
                  <label
                    class="block uppercase tracking-wide text-grey-darker text-xs font-bold mb-2"
                    :for="id+'_boolean_true'"
                  >{{ __('True value') }}</label>
                  <input
                    class="w-full form-control form-input form-input-bordered py-3 px-4"
                    id="id+'_boolean_true'"
                    :placeholder="__('True value')"
                    type="text"
                    v-model="column.trueValue"
                    autocomplete="off"
                  >
                </div>
                <div class="w-1/2 pr-3">
                  <label
                    class="block uppercase tracking-wide text-grey-darker text-xs font-bold mb-2"
                    :for="id+'_boolean_false'"
                  >{{ __('False value') }}</label>
                  <input
                    class="w-full form-control form-input form-input-bordered py-3 px-4"
                    id="id+'_boolean_false'"
                    :placeholder="__('False value')"
                    type="text"
                    v-model="column.falseValue"
                    autocomplete="off"
                  >
                </div>
              </div>
          ...
          <template
            v-if="column.field == 'Currency' || column.field == 'Date' || column.field == 'DateTime'"
          >
            <div class="flex flex-wrap">
              <div class="w-full flex flex-wrap content-start">
                <div class="w-full pr-3">
          ...
          <template
            v-if="column.field == 'File' || column.field == 'Image' || column.field == 'Avatar'"
          >
            <div class="flex flex-wrap">
              <div class="w-full flex flex-wrap content-start">
                <div class="w-full pr-3">
            ...
            <div class="flex flex-wrap">
              <div class="w-full flex flex-wrap content-start">
                ...
                <div class="w-full flex flex-wrap mt-6">
                  <label
                    class="block uppercase tracking-wide text-grey-darker text-xs font-bold mr-2"
                    :for="id+'_column_only_cities'"
                  >{{ __('Only Cities') }}</label>
            ...
            <div class="flex flex-wrap">
              <div class="w-full flex flex-wrap content-start">
                <div class="w-full flex flex-wrap mt-6">
                  <label
                    class="block uppercase tracking-wide text-grey-darker text-xs font-bold mr-2"
                    :for="id+'_column_loadingWhen'"
                  >{{ __('Loading When') }}</label>
                  <multiselect
                    v-model="column.loadingWhen"
                    :taggable="true"
                    :options="['waiting', 'running']"
                    :multiple="true"
                    @tag="addLoading"
                  ></multiselect>
                </div>
                <div class="w-full flex flex-wrap mt-6">
                  <label
                    class="block uppercase tracking-wide text-grey-darker text-xs font-bold mr-2"
                    :for="id+'_column_failedWhen'"
                  >{{ __('Failed When') }}</label>
                  <multiselect
                    v-model="column.failedWhen"
                    :taggable="true"
                    :options="['failed']"
                    :multiple="true"
                    @tag="addFailed"
            ...
            <div class="ml-0 flex items-center">
              <small>
                <a
                  ...
                  target="_blank"
                  :href="referenceUrl"
                >{{ __('Laravel Nova docs reference') }}</a>
              </small>
            </div>

            <button
              class="ml-auto btn btn-default btn-primary mr-3"
              v-on:click="modalExtras = false"
            >{{ __('Close') }}</button>
          </div>
        </div>
      </modal>
      ...
      <template v-if="column.field == 'Relation'">
        <button
          ...
          :class="{ 'btn-danger': relationFail, ' btn-primary': !relationFail }"
          v-if="column.field == 'Relation'"
          v-on:click="checkField"
        >{{ __('Set Relation') }}</button>
      </template>
    </div>
  </div>
</template>
